...
const pokeFetch = async (url: string) => {
  ...
  const jsonDBPath = path.resolve(__dirname, '../server/localCache/' + fullUrl + '.json');
  ...
  let pathStart = path.resolve(__dirname, '../server/localCache/' + baseUrl);
  // console.log('pathStart', pathStart);
  const allDirs = cleanUrl.split('/');
  const dirs = allDirs.filter((segment) => {
    return !!segment;
  });
  dirs.pop();
  //console.log('dirsFiltered', dirsFiltered);
